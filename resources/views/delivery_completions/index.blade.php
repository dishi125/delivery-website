...
@section('content')
    <section class="content-header">
        <h1 class="pull-left">Delivery Completions</h1>
        <h1 class="pull-right">
{{--           <a class="btn btn-primary pull-right" style="margin-top: -10px;margin-bottom: 5px" href="{{ route('deliveryCompletions.create') }}">Add New</a>--}}
        </h1>
    </section>
    <div class="content">
...
{{--                    @include('delivery_completions.table')--}}
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $(document).on('click', '.modaldata', function(event){
                ...
                $.ajax({
                    type: 'get',
                    url:  "{{url('/admin/deliverycompletemodaldata')}}"+"/"+id,
                    success: function (res) {
                        if (res.success == true) {
                            // alert(res.message);
                            $('#tbody-data').html(res.message);
                            $('#com').html(res.complete);
                            $('#all').html(res.all);
                            $('#DeliveryCompleteModal').modal('show');
                        }
                    }
                });
            });
            ...
            $(document).on('shown.bs.modal', '#DeliveryCompleteModal', function(event){
        ...
        function fetch_data()
        {
            ...
            jqxhr.abort();
            jqxhr=$.ajax({
                url:"{{ url('admin/deliverycomplete/fetch_data') }}",
                data:{
                    page:page,
                    per_page:per_page,
                    search:search,
